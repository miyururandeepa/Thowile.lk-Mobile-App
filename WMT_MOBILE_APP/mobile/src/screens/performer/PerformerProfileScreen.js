  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: COLORS.cream }}>
      <View style={styles.hero}>
        <View style={styles.avatar}><Text style={styles.avText}>{user?.name?.[0]}</Text></View>
        <Text style={styles.name}>{user?.name}</Text>
        <Text style={styles.role}>Performer</Text>
      </View>
      <View style={{ padding: SPACING.md, gap: SPACING.sm }}>
        <Card>
          {[['Email', user?.email], ['Phone', user?.phone], ['Role', user?.roleName]].map(([k,v]) => v ? (
            <View key={k} style={styles.row}><Text style={styles.key}>{k}</Text><Text style={styles.val}>{v}</Text></View>
          ) : null)}
        </Card>
